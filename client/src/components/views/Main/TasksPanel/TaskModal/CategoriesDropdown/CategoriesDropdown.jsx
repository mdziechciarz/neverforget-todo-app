  ...
  return (
    <div className={style.wrapper}>
      <div className={`${style.container} ${isOpen && style.open}`}>
        <Head isOpen={isOpen} handleOpen={handleOpen} selectedCategory={selectedCategory} />
        {isOpen && <Content categories={categories} selectCategoryId={selectCategoryId} setIsOpen={setIsOpen} />}
      </div>
    </div>
  )
}
...
const Content = ({ categories, selectCategoryId, setIsOpen }) => {
    ...
    <div className={style.content}>
      <ul className={style.elementsList}>
        {categories.map((category) => (
          ...
          <li key={category._id} className={style.element} onClick={() => handleSelect(category)}>{category.name}</li>
        ))}
      </ul>
